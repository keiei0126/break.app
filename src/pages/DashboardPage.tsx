    ...
    <div style={{ padding: '20px', maxWidth: '500px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '15px', margin: '20px 0' }}>
        ...
        <div style={{ flex: 1, padding: '15px', background: '#f6ffed', borderRadius: '8px', textAlign: 'center' }}>
          <h3>浮いた時間</h3>
          <p style={{ fontSize: '28px', fontWeight: 'bold' }}>約 {totalSavedMinutes} 分</p>
        </div>
      </div>
        ...
        <ul style={{ listStyle: 'none', padding: 0 }}>
          {logs.map((log) => (
            <li key={log.id} style={{ borderBottom: '1px solid #eee', padding: '10px 0' }}>
              <div><strong>{log.targetApp}</strong> をやめて 👉 <strong>{log.actionTitle}</strong></div>
              <small style={{ color: '#888' }}>{log.timestamp}（約{log.savedMinutes}分 節約）</small>
            </li>
          ))}
        </ul>
      )}
